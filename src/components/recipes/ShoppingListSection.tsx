...
import { ShoppingBag, Copy, Check, CheckSquare, Square } from 'lucide-react';
        ...
        <div className="space-y-4">
          ...
          {totalItems > 0 && (
            <div className="flex items-center justify-between text-sm text-muted-foreground">
              ...
              <span>{checkedCount}/{totalItems} ({Math.round((checkedCount / totalItems) * 100)}%)</span>
            </div>
          )}
              ...
              <div key={categoryIndex} className="space-y-2">
                ...
                <div className="space-y-2 pl-4">
                  {items.map((item, itemIndex) => {
                    const globalIndex = Object.values(categories)
                      .flat()
                      .indexOf(item);
                    ...
                    return (
                      <div
                        key={itemIndex}
                        className={`flex items-center space-x-3 p-2 rounded-lg hover:bg-muted/50 transition-all cursor-pointer ${
          ...
          <div className="bg-muted/30 rounded-lg p-3 text-xs text-muted-foreground">
            <p>
              💡 <strong>Wskazówka:</strong> Zaznaczaj produkty podczas zakupów. 
              Lista jest zorganizowana według działów sklepowych dla Twojej wygody.
            </p>
          </div>
        </div>
      </CardContent>
